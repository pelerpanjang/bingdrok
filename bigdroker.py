...
def clean():
	...
	print("[{}+{}] situs yang sama berhasil di hapus".format(settings.r,settings.y))
	print("\n[{}+{}] file saved as {}sites.txt{}!".format(settings.r,settings.y,settings.b,settings.y))
...
print("""{}
____  _               ____            _
| __ )(_)_ __   __ _  |  _ \ _ __ ___ | | _____ _ __
|  _ \| | '_ \ / _` | | | | | '__/ _ \| |/ / _ \ '__|
| |_) | | | | | (_| | | |_| | | | (_) |   <  __/ |
|____/|_|_| |_|\__, | |____/|_|  \___/|_|\_\___|_|
               |___/
...
dorklist = raw_input("[{}*{}] list drok: ".format(settings.r,settings.y))

try:
	dorks = open(dorklist, 'r').read().splitlines()
	print("[{}+{}] droking di mulai plase wait peler.. :)".format(settings.r,settings.y))
	pp = Pool(10)
	pr = pp.map(dorkscan,dorks)
except:
	print("[{}-{}] file nya gak ada_-!".format(settings.r,settings.y))
